...
exports.dealerReport = (req, res) => {
    ...
    CAST(CAST(SUM(p.amount) AS double) AS decimal(10,2)) AS 'Paybackperiod_Amount', m.amount AS 'Membership_Fee', CAST(CAST((IFNULL(p.amount,0) + IFNULL(m.amount,0) + IFNULL(i.amount,0)) AS double) AS decimal(10,2)) AS 'Total_Sum'
    FROM client c
